...
const VerifikatorDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch recent surat
      const suratResponse = await api.get('/verifikator/surat');
  ...
  const getStatusBadge = (status) => {
    const statusConfig = {
      'menunggu_rt': { color: 'bg-yellow-50 text-yellow-700 border-yellow-200', text: 'Menunggu RT' },
      'menunggu_rw': { color: 'bg-blue-50 text-blue-700 border-blue-200', text: 'Menunggu RW' },
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-semibold text-gray-900 text-sm truncate">
                        {surat.nama_jenis_surat || surat.jenis_surat?.nama_surat || 'Surat'}
                      </h4>
                      <p className="text-xs text-gray-500 mt-1 flex items-center gap-1">
                        <FiUser className="w-3 h-3" />
                        {surat.warga?.nama || 'Warga'}
                      </p>
                      <p className="text-xs text-gray-400 mt-1">
                        {formatDate(surat.created_at)}
                      </p>
                      <div className="mt-2">
                        {getStatusBadge(surat.status_surat || surat.status)}
                      </div>
                    </div>
